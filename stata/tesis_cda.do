...
*   colapsa el tiempo a un indice secuencial y se corre newey. Los resultados
...
label var Iliq     "Iliquidez (dolarizacion, %)"
...
tsline DR, lcolor(navy) lwidth(medthick)                                       ///
    ...
    title("Etapa 1 - Diferencial de retorno DR (Gs vs USD)")                   ///
    subtitle("marca roja = quiebre de regimen 2011 (Metas de Inflacion)")      ///
    ytitle("DR (pp, trimestral)") xtitle("Trimestre")                          ///
    name(g_DR, replace)
graph export "output/figuras/etapa1_DR_serie.png", replace width(1400)

tsline DR DRcons, lcolor(navy orange) lpattern(solid dash)                     ///
    yline(0, lpattern(dot) lcolor(gs8))                                        ///
    xline(`=yq(2011,1)', lcolor(red))                                          ///
    title("Etapa 1 - DR vs DRcons") legend(order(1 "DR (estandar)" 2 "DRcons (conservador)")) ///
    name(g_DRc, replace)
graph export "output/figuras/etapa1_DR_vs_DRcons.png", replace width(1400)
...
*   => la composicion CAMBIO tras 2011; la iliquidez PIERDE peso (direccion
*      inversa a la conjetura especifica de migracion hacia la iliquidez).
...
*   Resultado: ADF residuos = -4.77 (p<0.001), por debajo de los valores criticos
*   de Engle-Granger (~ -3.4 al 5% con 2 regresores, T~80) => COINTEGRAN.
*   Esta es la respuesta formal al posible cuestionamiento de "regresion espuria".
